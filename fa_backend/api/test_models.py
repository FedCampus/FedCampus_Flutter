...
class ModelCreationTest(TestCase):
    ...
    def test_record_dp_creation(self):
        record_dp = RecordDP.objects.create(
            user=self.user,
            startTime=123456789,
            endTime=123456999,
            dataType="testTypeDP",
...
class CustomerLogModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.log = Log.objects.create(user=self.user, file=dummy_file)
    ...
    def test_log_str(self):
        ...
        self.assertIn(
            timezone.localtime(self.log.time).strftime("%Y-%m-%d %H:%M:%S"),
            self.log.__str__(),
        )
