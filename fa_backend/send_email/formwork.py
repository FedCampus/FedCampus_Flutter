...
def sendmail(content, netid, subject, from_email="fedcampus@dukekunshan.edu.cn", test=False):
    """
    ...
    send_mass_mail(((subject, c, from_email, [n + "@duke.edu"]) for c, n in zip(content, netid)))
...
def main(args):
    ...
    users_df = filter_users(args.excel_file, grade_exclusion=2024, status_required="student")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Send emails to filtered users from an Excel file.")
    parser.add_argument("--excel_file", required=True, help="The path to the Excel file.")
    parser.add_argument("--text_file", required=True, help="The path to the text file with the email content.")
    parser.add_argument("--email_action", choices=["test", "send"], default="test", help="Whether to test or send emails.")
