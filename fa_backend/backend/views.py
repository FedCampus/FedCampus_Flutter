...
def mainPage(request):
    ...
    userCount = {'number': Customer.objects.count(),}
    ...
    df = pd.DataFrame({
        "Date": active_users.keys(),
        "activeUsers": active_users.values()
    })
    activeUserFig = px.bar(df, x="Date", y="activeUsers", title="Active Users in recent 14 days")
    template_activeUserFig = plot(activeUserFig, output_type="div")

    # The context dictionary to pass in
    context = {
        "userCount": userCount,
        "warningList": OrderedDict(sorted(inactive_users.items(), key=lambda x: x[1])), #Organized the increasing
